...
Если взять наш скрипт, то все системные вызовы обслуживаются напрямую хостовым ядром. Аналогично в Docker. А вот gVisor добавляет Sentry в юзерспейсе. И системные вызовы обрабатывюатся уже не хостовым ядром (большая часть), а Sentry (режим systrap). Благодаря этому уменьшается поверхность атаки, которые связаны с уязвимостями хостового ядра.
